/**
 * Ähnlichkeitsberechnung und Rangfolge.
 *
 * Bewusst ein exakter Durchlauf über alle Abschnitte eines Notebooks statt
 * eines Vektor-Index. Die Begründung steht nicht hier als Behauptung, sondern
 * im README als Messung: scripts/measure-search.ts fährt die Suche gegen 100,
 * 1.000 und 10.000 Abschnitte und gibt Laufzeit und übertragene Datenmenge
...
/**
 * Kosinus-Ähnlichkeit zweier Vektoren, Wertebereich -1 bis 1.
 *
 * Kosinus und nicht euklidischer Abstand, weil nur die Richtung des Vektors die
 * Bedeutung trägt; die Länge hängt an der Textlänge und würde lange
 * Abschnitte systematisch bevorzugen.
 *
 * Zwei Sonderfälle geben 0 zurück statt zu werfen: unterschiedliche
 * Dimensionen (ein Abschnitt wurde mit einem anderen Modell eingebettet) und
 * ein Nullvektor (Division durch null). "Nicht ähnlich" ist die sichere
...
export function cosineSimilarity(a: readonly number[], b: readonly number[]): number {
  ...
  // das den Unterschied zwischen einem und drei Durchläufen über 7,7 Mio.
  // Zahlen.
...
/**
 * Sortiert Abschnitte nach Ähnlichkeit zur Frage und gibt die besten zurück.
 *
 * `getEmbedding` statt eines festen Feldnamens, damit die Funktion nicht weiß,
...
export function rankBySimilarity<T>(
  ...
  for (const chunk of chunks) {
    ...
    // Abschnitte ohne jede Ähnlichkeit gar nicht erst einsortieren.
