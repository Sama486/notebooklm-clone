...
describe('Sperrliste fuer ausgehende Adressen', () => {
  it('sperrt Loopback in allen Schreibweisen', () => {
    // Nicht nur 127.0.0.1: das ganze /8 gehoert dazu, und die Dezimalform
    // 2130706433 loest der URL-Parser ebenfalls nach 127.0.0.1 auf.
  ...
  it('sperrt den Cloud-Metadaten-Bereich', () => {
    // 169.254.169.254 ist bei AWS, GCP und Azure der Endpunkt, ueber den sich
  ...
  it('sperrt IPv4-Adressen, die in IPv6 versteckt sind', () => {
    // Der haeufigste Umgehungsversuch: dieselbe Maschine, andere Schreibweise.
    // Ohne das Auspacken stuende keine dieser Adressen in einer IPv6-Sperrliste.
  ...
  it('laesst oeffentliche Adressen durch', () => {
    ...
    // Eine oeffentliche IPv4 in IPv6-Verpackung bleibt oeffentlich.
    expect(isBlockedAddress('::ffff:1.1.1.1')).toBe(false);
  });

  it('sperrt alles, was sich nicht als Adresse lesen laesst', () => {
    // Der Standardwert ist "gesperrt", nicht "erlaubt". Was wir nicht
    // einordnen koennen, rufen wir nicht ab.
